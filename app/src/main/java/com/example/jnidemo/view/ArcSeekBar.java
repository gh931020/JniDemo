 ...
 *
 */
public class ArcSeekBar extends View implements BaseBarInterface {
     ...
     * 显示的最小值
     */
    private float mMinValue = 4;
    /**
     * 显示的最大值
     */
    private float mMaxValue = 44;
    ...
    private void init(Context context, AttributeSet attrs) {
        ...
        for (int i = 0; i < size; i++) {
            ...
            if (attr == R.styleable.ArcSeekBar_arcStrokeWidth) {
                ...
                float max = a.getFloat(attr, mMax);
                if (max > 0) {
                    mMax = max;
                }
            }
            ...
            else if (attr == R.styleable.ArcSeekBar_arcHasGap) {
                mHasGap = a.getBoolean(attr, false);
            }
            else if (attr == R.styleable.ArcSeekBar_arcGapNum) {
                mGapNum = a.getInt(attr, mGapNum);
            }
            else if (attr == R.styleable.ArcSeekBar_arcGapAngle) {
                mGapAngle = a.getFloat(attr, mGapAngle);
            }
            else if (attr == R.styleable.ArcSeekBar_arcShowValue) {
                isShowValue = a.getBoolean(attr, false);
            }
            else if (attr == R.styleable.ArcSeekBar_arcMinValue) {
                mMinValue = a.getFloat(attr, mMinValue);
            }
            else if (attr == R.styleable.ArcSeekBar_arcMaxValue) {
                mMaxValue = a.getFloat(attr, mMaxValue);
            }
            else if (attr == R.styleable.ArcSeekBar_arcValueTextSize) {
                mValueTextSize = a.getDimension(attr, mValueTextSize);
            }
            else if (attr == R.styleable.ArcSeekBar_arcValueTextColor) {
                mValueTextColor = a.getColor(attr, mValueTextColor);
            }
            else if (attr == R.styleable.ArcSeekBar_arcUnitText) {
    ...
    private void drawText(Canvas canvas) {
        ...
        if (isShowValueText) {
            ...
            canvas.drawText(value + mUnitText, textBaseX, textBaseY, mTextPaint);
    ...
    private void drawValue(Canvas canvas) {
        ...
        String minValue = DecimalFormat.getIntegerInstance().format(mMinValue);
        String minValueMeasure = "00";
    ...
    private void updateDragThumb(float x, float y, boolean isSingle) {
        float progress = getProgressForAngle(getTouchDegrees(x, y));
        if (!isSingle) {
    ...
    private int getProgressForAngle(float angle) {
        return Math.round(mMax / mSweepAngle * angle);
    }
     ...
     * @param duration 动画时长
     */
    public void showAnimation(float from, int to, int duration) {
     ...
     * @param listener
     */
    public void showAnimation(float from, int to, int duration, Animator.AnimatorListener listener) {
    ...
    private float getRatio() {
        Log.e(TAG, "getRatio: mProgress = " + mProgress);
    ...
    private int getCurrentValue(){
        return (int) Math.ceil (mMinValue + (mMaxValue - mMinValue) * mProgressPercent / mMax);
    }
    ...
    @Override
    public void reduceProcess(boolean flag) {
        reduceProcess(flag, 1);
    }

    @Override
    public void addProcess(boolean flag) {
        addProcess(flag, 1);
    }
    ...
    private void reduceProcess(boolean flag, int stepValue) {
        if (mProgress > 0) {
            ...
            float progress = Math.max(0, mProgress - stepValue);
    ...
    private void addProcess(boolean flag, int stepValue) {
        if (mProgress < mMax) {
            // float progress = Math.min(mMax, mProgress + mMax / (mMaxValue - mMinValue));
            float progress = Math.min(mMax, mProgress + stepValue);
